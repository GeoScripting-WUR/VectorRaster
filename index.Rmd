...
In this previous vector tutorial we used *sf*, but another popular package that handles vector data in R is *sp*. *Sp* is similar to *sf*, but is able to work with vector data and raster data through the *raster* package. In the current lesson, we'll see what can be done when the two worlds of vector data and raster data intersect. Secondly we will learn how to change *sf* objects to *sp* objects.
...
plot(wagLandsatCrop$band1, alpha = 0.5, add = T)
