...
# the bbox must be converted to a simple feature collection
wagLandsatCropBbox <- st_as_sfc(st_bbox(wagLandsatCrop))

# write the bounding box to a kml file
outKML <- paste0(dataDir,"wagLandsatCropBbox.kml")
## the ifelse() checks for existence of the kml file and sets the delete_dsn option to true or false accordingly
st_write(wagLandsatCropBbox, outKML, driver = "kml", delete_dsn = ifelse(file.exists(outKML),TRUE,FALSE))
```
