...
The example uses the same data as you used in the [exercise](http://geoscripting-wur.github.io/IntroToRaster/#exercise-design-a-pre-processing-chain-to-assess-change-in-ndvi-over-time) of the raster tutorial.
...
try(st_write(water, "output/water.kml", driver = "kml", delete_dsn = ifelse(file.exists(outKML), TRUE, FALSE)))
...
```{r}
# Download data
bel <- rast(getData('alt', country='BEL', mask=TRUE))
...
Let's first write this line to a file, using a few different file formats. Note that `draw()` returns a SpatVector. That means we cannot (directly) use `st_write()` here like we did before. However, one option is making use of `writeVector()` from the terra package. After writing to disk, you might try opening these new files in R, or in different software, such as QGIS or ArcGIS.

```{r}
# To a GeoJSON
writeVector(line, 'output/line.json')

# To an ESRI Shapefile
writeVector(line, 'output/line.shp')
```

Now the elevation values can simply be extracted using `extract()`. Note the use of the `along=` argument which ensures that the samples remain in order along the segment.

```{r}
alt <- extract(bel, line)
```

We can plot the result as follows. However, note that the x axis does not really provide any indication of distance, as not all cells are equally distant from each other!
