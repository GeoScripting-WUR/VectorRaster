...
- Buffer:
  - `st_buffer()` Computes a buffer around simple feature geometries of `sfg`, `sfc` and `sfc` objects, returning an object of the same class.
  - Buffer distance parameter is in map units (typically meters).
  - See `?st_buffer` for more info.
...
plotRGB(wagLandsat, 5, 4, 3, stretch = 'lin')
```
...
```{r, eval=FALSE, fig.align='center', fig.show='hold'}
...
plotRGB(wagLandsatCrop, 5, 4, 3, stretch = 'lin')
plotRGB(wagLandsatSub, 5, 4, 3, stretch = 'lin')
...
# Plot
plotRGB(wagLandsatSubW, 5, 4, 3, stretch = 'lin')
...
We want to look at a transect, which we can draw by using two points. We will use the `st_linestring()` function to draw the line between two coordinates.


```{r, eval=FALSE}
# Create two coordinates, start and end of the transect
p1 <- c(2.75, 51.25) # (lat, lon)
p2 <- c(6, 49.75)

# Matrix of the coordinates
coords <- rbind(p1, p2)

# Create the transect between p1 and p2
line <- st_sfc(st_linestring(coords), crs = "EPSG:4326") # st_sfc adds a crs
line_sf <- st_sf(line) # transform to spatial feature object
```

```{r, eval=FALSE, echo=FALSE, fig.align='center'}
plot(bel)
plot(line_sf, add=TRUE)
```

Let's first write this line to a file, using a few different file formats. We can use `st_write()` here like we did before. After writing to disk, you might try opening these new files in R, or in different software, such as QGIS or ArcGIS.

```{r, eval=FALSE}
# To a GeoJSON
st_write(line_sf, 'output/line.geojson')

# To an ESRI Shapefile
st_write(line_sf, 'output/line.shp')
```

Now the elevation values can simply be extracted using `extract()`.

```{r, eval=FALSE}
alt <- extract(bel, line_sf)
```
