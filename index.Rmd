...
- Predict the land cover on the Landsat image
...
nlCity <- raster::getData('GADM',country='NLD', level=2) ## level 3 gives an error!!!
class(nlCity)
...
```{r}
nlCity@data <- nlCity@data[!is.na(nlCity$NAME_2),] ## trick to remove rows with NA
...
> Note the use of `df = TRUE` in the `extract()` call, so that we get a data frame in return. Data frame are the most common class to work with all types of model, such as linear models (`lm()`) or random forest models as we use later.
...
[product details](https://lpdaac.usgs.gov/dataset_discovery/modis/modis_products_table/mod13a3)
